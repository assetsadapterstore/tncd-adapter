...
func (decoder *TransactionDecoder) CreateRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	var (
		...
		accountBalance *futurepia.ApiBalance
		amountStr      string
		to             string
	)
...
func (decoder *TransactionDecoder) SubmitRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) (*openwallet.Transaction, error) {
	...
	var stx *futurepia.TransConMain
	txHex, err := hex.DecodeString(rawTx.RawHex)
	if err != nil {
	...
	tranSub := &futurepia.TransConMainSub{
...
func (decoder *TransactionDecoder) createRawTransaction(
	...
	var params = make([]*futurepia.ParamData, 0)
	params = append(params, &futurepia.ParamData{
		From: accountName,
		To:   to,
		Memo: memo,
		Amount: eos.Asset{
	...
	var paramss = make([][]*futurepia.ParamData, 0)
	paramss = append(paramss, params)
	now := time.Now()
	...
	transCoin := &futurepia.TransConMain{
	...
	//t := eos.Int64(amountDec.Shift(int32(decoder.wm.Decimal())).IntPart())/100000000
	txdata[11] = byte(0)
